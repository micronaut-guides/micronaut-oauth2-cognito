...
<1> The provider identifier should match the last part of the url you entered as a redirect url `/oauth/callback/cognito`
<2> Client Secret. See previous screenshot.
<3> Client ID. See previous screenshot.
<4> `issuer` url. It allows micronaut to discover the configuration of the OpenID Connect server. Note: we will use the pool id and region mentioned previously.
